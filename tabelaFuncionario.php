...
if(isset($_POST['pesquisar'])){
  ...
  $tabela = $pdo->prepare("SELECT nome_funcionario, dtNasc_funcionario, email_funcionario, telefone_funcionario, data_funcionario FROM tb_funcionarios WHERE nome_funcionario LIKE :nome");
  $tabela->bindValue(':nome', '%' . $nome . '%');
  $tabela->execute();
  $rowTabela = $tabela->fetchAll();
} else {
  $tabela = $pdo->prepare("SELECT nome_funcionario, dtNasc_funcionario, email_funcionario, telefone_funcionario, data_funcionario FROM tb_funcionarios");
  $tabela->execute();
  $rowTabela = $tabela->fetchAll();
}
...
<div class="logo-details"><label for="">ㅤㅤㅤ</label>
      ...
      <ul class="nav-links">
        ...
        <li>
          <a href="tabelaFuncionario.php"  class="active">
          <i class='bx bx-group'></i>
            ...
            <label for="nome_pesquisa" class="form-label" style="font-weight: bold;">Pesquisar por Nome</label>
              <div class="input-group" style="margin-bottom: 10px;">
                <form method="POST">
                  ...
                  <button class="btn btn-primary" type="button" name="resetar" onclick="window.location.href = 'tabelaFuncionario.php'" style="background-color: #081D45; color: #fff; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer;">Limpar</button>
                  ...
                  <table class="table table-white">
                    <thead>
                      <tr>
                        ...
                        <th scope="col">Email Funcionário</th>
                    ...
                    <tbody>
                      ...
                      if (!empty($rowTabela)){
                        foreach($rowTabela as $linha){
                          ...
                          echo "<td>".$linha['dtNasc_funcionario']."</td>";
                          echo "<td>".$linha['email_funcionario']."</td>";
                          echo "<td>".$linha['telefone_funcionario']."</td>";
                          echo "<td>".$linha['data_funcionario']."</td>";
                          echo '</tr>';
                        }
                      } else {
